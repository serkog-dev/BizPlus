...
async function main() {
  ...
  for (let i = 0; i < providers.length; i++) {
    ...
    // Connect provider to all services
  ...
  // Schedules - Sunday through Friday, 09:00-19:00, Saturday closed
  for (const provider of createdProviders) {
    for (let day = 0; day <= 6; day++) {
      const isWorking = day !== 6 // Saturday closed
